...
class FilesController {
  static async postUpload(request, response) {
    ...
    if (fileParentId !== 0) {
      const parentFile = await dbClient.database.collection('files').findOne({ _id: ObjectId(fileParentId) });
    ...
    if (['folder'].includes(fileType)) {
      await dbClient.database.collection('files').insertOne(fileDataDb);
  ...
  static async getShow(request, response) {
    ...
    const fileDocument = await dbClient.database.collection('files').findOne({ _id: ObjectId(idFile), userId: user._id });
  ...
  static async getIndex(request, response) {
    ...
    const files = await dbClient.database.collection('files').aggregate(aggregateData);
    const filesArray = [];
  ...
  static async putPublish(request, response) {
    ...
    let fileDocument = await dbClient.database.collection('files').findOne({ _id: ObjectId(idFile), userId: user._id });
    if (!fileDocument) return response.status(404).send({ error: 'Not found' });

    await dbClient.database.collection('files').update({ _id: ObjectId(idFile) }, { $set: { isPublic: true } });
    fileDocument = await dbClient.database.collection('files').findOne({ _id: ObjectId(idFile), userId: user._id });
  ...
  static async putUnpublish(request, response) {
    ...
    await dbClient.database.collection('files').update({ _id: ObjectId(idFile), userId: user._id }, { $set: { isPublic: false } });
    fileDocument = await dbClient.database.collection('files').findOne({ _id: ObjectId(idFile), userId: user._id });
  ...
  static async getFile(request, response) {
    ...
    const fileDocument = await dbClient.database.collection('files').findOne({ _id: ObjectId(idFile) });
    if (!fileDocument) return response.status(404).send({ error: 'Not found' });
    ...
    if (token) {
      const redisToken = await redisClient.get(`auth_${token}`);
      if (redisToken) {
        user = await dbClient.database.collection('users').findOne({ _id: ObjectId(redisToken) });
